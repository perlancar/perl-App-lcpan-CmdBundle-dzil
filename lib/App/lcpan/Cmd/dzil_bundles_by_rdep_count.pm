package App::lcpan::Cmd::dzil_bundles_by_rdep_count;
...
$SPEC{'handle_cmd'} = {
    v => 1.1,
    summary => 'List Dist::Zilla bundles ranked by number of reverse dependencies',
    args => {
...
sub handle_cmd {
    ...
    my @where;
    my @binds;
    push @where, "(name LIKE 'Dist::Zilla::PluginBundle::%')";
    if ($args{author}) {
        push @where, "(author=?)";
...
";
    ...
    while (my $row = $sth->fetchrow_hashref) {
        $row->{name} =~ s/^Dist::Zilla::PluginBundle:://;
        push @res, $row;
    }
    my $resmeta = {};
